...
DEFAULT_DATA = {
    ...
    "petty_cash": {
        ...
        "limit": 100000,
        "items": [
            {"id": 1, "title": "中庭景觀植栽修剪與枯枝清運", "amount": 15000, "category": "公設維護", "invoice": "INV-202609-0821", "date": "2026-09-10"},
            {"id": 2, "title": "地下室水塔清洗前置濾心更換", "amount": 8500, "category": "機電保養", "invoice": "INV-202609-0914", "date": "2026-09-12"},
            {"id": 3, "title": "各棟逃生梯緊急照明燈電池檢修更新", "amount": 6200, "category": "消防安全", "invoice": "INV-202609-1102", "date": "2026-09-14"},
            {"id": 4, "title": "社區管理室門禁卡與防磁貼片批次採購", "amount": 4800, "category": "事務消耗", "invoice": "INV-202609-1198", "date": "2026-09-15"},
            {"id": 5, "title": "A/B棟公共梯廳節能感應LED燈管補料", "amount": 3200, "category": "修繕五金", "invoice": "INV-202609-1240", "date": "2026-09-16"}
        ],
        "approvals": {
            "director": {"role": "主任委員", "name": "陳建宏", "status": "approved", "comment": "各項費用單據核對無誤，同意核銷撥款。", "time": "2026-09-17 10:15"},
            "finance": {"role": "財務委員", "name": "林秀玲", "status": "approved", "comment": "單據齊全，未逾零用金10萬元上限，同意。", "time": "2026-09-17 11:30"},
            "supervisor": {"role": "監察委員", "name": "王國華", "status": "approved", "comment": "已完成五項單據審核無誤，全額核准。", "time": "2026-09-17 18:15"}
        },
...
def generate_ai_response(msg, role, unit, data):
    ...
    # 4. 零用金審核相關
    if "零用金" in msg or "審核" in msg or "報表" in msg:
        petty = data["petty_cash"]
        total_spent = sum(i["amount"] for i in petty["items"])
        remaining = petty["limit"] - total_spent
        top3 = sorted(petty["items"], key=lambda x: x["amount"], reverse=True)[:3]
        top3_str = "\n".join([f"  {idx+1}. {item['title']}：NT$ {item['amount']:,}" for idx, item in enumerate(top3)])
        status_text = "已全額核准結算" if petty["final_status"] == "approved" else ("審核中 (等待委員同意)" if petty["final_status"] == "in_review" else "已被退回補正")
        return (
            f"【大清天朵二期 零用金審核精煉摘要】\n"
            f"📅 結算月份：{petty['month']}\n"
            f"💰 總支出金額：NT$ {total_spent:,} 元\n"
            f"💵 零用金剩餘：NT$ {remaining:,} 元 (上限 10 萬元)\n"
            f"📊 最大筆開銷前三名：\n{top3_str}\n"
            f"📝 目前審核狀態：{status_text}\n"
            f"請至「零用金審核」頁籤檢視各委員簽核進度與憑證單據。"
        )
    ...
    return (
        ...
        "我能協助您處理：\n"
        "1. 規約與決議邏輯檢核（含走廊鞋櫃、地下室防水閘門檢測與定期操演規約）\n"
        "2. 防汛演練與氣象局連線（每年5月自動提醒演練、中央氣象局豪大雨主動推播）\n"
        "3. 影像辨識與設施派單（垃圾棄置、遺失物、修繕報修）\n"
        "4. 社區財務自動銷帳（銀行入帳自動比對、住戶個人收據）\n"
        "5. 零用金三委員多重簽核流程（精煉重點摘要、核准書產出）\n"
